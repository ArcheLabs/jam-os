import type { FileSystemRuntime, NameRuntime } from "../../runtime/types";
...
function renderFile(data: Uint8Array, path: string, jamPage: boolean, serviceId?: string): string { const mime = path.endsWith(".html") || path.endsWith(".htm") ? "text/html" : path.endsWith(".css") ? "text/css" : "text/plain"; if (mime === "text/plain") return `<pre class="raw-file">${escapeHtml(new TextDecoder().decode(data))}</pre>`; let html = new TextDecoder().decode(data).replace(/<jam-link\s+[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/jam-link>/gi, (_all, href, text) => `<a data-jam-link="${escapeAttr(href)}" href="${escapeAttr(href)}">${text}</a>`); html = sanitizeHtml(html); const bridge = jamPage ? `<script>(function(){document.addEventListener('click',function(e){var a=e.target.closest('[data-jam-link]');if(a){e.preventDefault();parent.postMessage({type:'jam:navigate',href:a.getAttribute('data-jam-link')},'*')}})})()</script>` : ""; const base = `<base href="${escapeAttr(path.endsWith("/") ? path : `${path.slice(0, path.lastIndexOf("/") + 1)}`)}">`; void serviceId; return `${base}${html}${bridge}`; }
function sanitizeHtml(html: string) { const candidate = DOMPurify as unknown as { sanitize?: (value: string, options: Record<string, unknown>) => string }; if (typeof candidate.sanitize === "function") return candidate.sanitize(html, { ADD_ATTR: ["data-jam-link"], FORBID_TAGS: ["script", "iframe", "object", "embed"], FORBID_ATTR: ["srcset"] }); return html.replace(/<script[\s\S]*?<\/script>/gi, "").replace(/<iframe[\s\S]*?<\/iframe>/gi, "").replace(/<\/?(?:object|embed)(?:\s[^>]*)?>/gi, "").replace(/\son[a-z]+\s*=\s*("[^"]*"|'[^']*')/gi, ""); }
