...
export function Browser({ runtime, serviceId }: { runtime: Runtime; serviceId: string | null }) {
  ...
  const navigate = useCallback(async (raw: string, push = true) => { setError(""); try { const url = canonicalizeUrl(raw); const resolved = await resolveBrowserUrl(url, { localFs: fs, names: runtime.names, siteForService: (targetServiceId) => runtime.computer.fs(targetServiceId) }); setAddress(url); setDoc(resolved); if (push) { setHistory((old) => [...old.slice(0, cursor + 1), url]); setCursor((old) => old + 1); } } catch (e) { setError(e instanceof Error ? e.message : "Unable to load page"); } }, [cursor, fs, runtime.names, runtime.computer]);
