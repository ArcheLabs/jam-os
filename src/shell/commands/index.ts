...
export async function executeCommand(ctx: ShellContext, input: string): Promise<CommandResult> {
  ...
  try {
    switch (command) {
      ...
      case "cd": { const target = path(args[0] || "/home/user"); const stat = await ctx.fs!.stat(target); if (!stat || stat.type !== "directory") return { exitCode: 1, stderr: `${target}: not a directory` }; return { exitCode: 0, stdout: `__CWD__${target}` }; }
      case "cat": return { exitCode: 0, stdout: await ctx.fs!.readText(path(args[0])) };
      case "write": if (args.length < 2) return { exitCode: 2, stderr: "usage: write <path> \"text\"" }; await ctx.fs!.writeText(path(args[0]), args.slice(1).join(" ")); return { exitCode: 0, stdout: "Stored on JAM." };
      case "touch": await ctx.fs!.writeText(path(args[0]), ""); return { exitCode: 0, stdout: "Stored on JAM." };
      ...
      case "status": { const network = await ctx.runtime.network.getInfo(); return { exitCode: 0, stdout: `Network: ${network.name}\nMode: ${ctx.runtime.mode.toUpperCase()}\nComputer: ${ctx.computerServiceId ? `#${ctx.computerServiceId}` : "none"}\nBackend: ${network.source}\nHealth: ${network.healthy ? "healthy" : "offline"}` }; }
      case "name": { if (args[0] === "claim") { const record = await ctx.names.claim(args[1], ctx.computerServiceId!); return { exitCode: 0, stdout: `Name claimed: ${record.name}` }; } if (args[0] === "bind") { const record = await ctx.names.bind(args[1], args[2] === "self" || !args[2] ? ctx.computerServiceId! : args[2]); return { exitCode: 0, stdout: `Name bound: jam://${record.name}` }; } if (args[0] === "show") { const record = await ctx.names.resolve(args[1]); return { exitCode: 0, stdout: `${record.name} → ${record.serviceId}` }; } return { exitCode: 2, stderr: "usage: name claim|show|bind <name>" }; }
      case "site": { if (args[0] === "publish") { const manifest = await ctx.fs!.publish(path(args[1] || "/home/user/Sites/home")); return { exitCode: 0, stdout: `Published ${Array.isArray(manifest.files) ? manifest.files.length : Object.keys(manifest.files).length} file(s).` }; } if (args[0] === "status") { const manifest = await ctx.fs!.manifest(); return { exitCode: 0, stdout: manifest ? `Published ${Array.isArray(manifest.files) ? manifest.files.length : Object.keys(manifest.files).length} file(s).` : "No site published." }; } if (args[0] === "open") { ctx.openApp("browser", "jam://alice"); return { exitCode: 0 }; } return { exitCode: 2, stderr: "usage: site status|publish [directory]|open" }; }
      case "service": { if (args[0] === "inspect" && ctx.runtime.services.inspect) { const result = await ctx.runtime.services.inspect(args[1]); return { exitCode: 0, stdout: JSON.stringify(result, null, 2) }; } if (args[0] === "call" && ctx.runtime.services.call) { const result = await ctx.runtime.services.call(args[1], new TextEncoder().encode(JSON.stringify({ op: "service:call", payload: args.slice(2).join(" ") })), ctx.account); return { exitCode: 0, stdout: new TextDecoder().decode(result) }; } return { exitCode: 2, stderr: "usage: service inspect|call <id>" }; }
