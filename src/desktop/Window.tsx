...
export function Window({ window: item, active = false, onFocus, onMove, onResize, onMinimize, onMaximize, onClose, children }: { window: WindowInstance; active?: boolean; onFocus: () => void; onMove: (x: number, y: number) => void; onResize: (width: number, height: number) => void; onMinimize: () => void; onMaximize: () => void; onClose: () => void; children: ReactNode }) {
  ...
  const startDrag = (event: React.PointerEvent) => { if (item.maximized || (event.target as HTMLElement).closest("button, input, select, textarea, a, .window-controls")) return; onFocus(); drag.current = { x: event.clientX, y: event.clientY, ox: item.x, oy: item.y }; (event.currentTarget as HTMLElement).setPointerCapture(event.pointerId); };
  ...
  const toggleMaximize = (event: React.MouseEvent) => { if ((event.target as HTMLElement).closest("button")) return; onMaximize(); };
