...
export class JamFileSystem {
  ...
  async stat(path: string): Promise<FileNode | DirectoryNode> { return parseBytes(await this.client.readService(this.serviceId, jsonBytes({ op: "fs:stat", path: normalizePath(path) }))); }
  async list(path: string): Promise<ListedNode[]> { return parseBytes(await this.client.readService(this.serviceId, jsonBytes({ op: "fs:list", path: normalizePath(path) }))); }
  async read(path: string): Promise<Uint8Array> { const result = parseBytes<{ bytes: number[] }>(await this.client.readService(this.serviceId, jsonBytes({ op: "fs:read", path: normalizePath(path) }))); return new Uint8Array(result.bytes); }
  async readText(path: string): Promise<string> { return new TextDecoder().decode(await this.read(path)); }
  async write(path: string, content: string | Uint8Array, mime = mimeFor(path)) { await this.client.invokeService(this.serviceId, jsonBytes({ op: "fs:write", path: normalizePath(path), bytes: [...bytes(content)], mime })); }
  ...
  async publish(path = "/home/user/Sites/home"): Promise<JamSiteManifest> { return parseBytes(await this.client.invokeService(this.serviceId, jsonBytes({ op: "site:publish", path: normalizePath(path) })).then((r) => r.output)); }
  async manifest(): Promise<JamSiteManifest | null> { return parseBytes(await this.client.readService(this.serviceId, jsonBytes({ op: "site:manifest" }))); }
  async readPublished(path: string): Promise<{ bytes: Uint8Array; mime: string }> { const result = parseBytes<{ bytes: number[]; mime: string }>(await this.client.readService(this.serviceId, jsonBytes({ op: "site:read", path: normalizePath(path) }))); return { bytes: new Uint8Array(result.bytes), mime: result.mime }; }
