...
export class MiniJamTransport {
  ...
  async build(_input: CompileInput): Promise<CompileOutput> { throw new JamProtocolError("Production Service artifacts must be built through the JamScript build interface", "LOCAL_BUILD_REQUIRED"); }
  ...
  async submitWorkOperation(serviceId: string, requestBytes: Uint8Array, _account: AccountInfo, _signer: AccountAdapter, extrinsics: Uint8Array[] = []): Promise<Operation> { const service = await this.getService(serviceId); if (!service.codeHash) throw new JamProtocolError("VITE_COMPUTER_SERVICE_CODE_HASH must identify the locally built artifact", "SERVICE_CODE_HASH_REQUIRED"); const context = await this.rpc<{ blockHash: string; stateRoot: string; slot: number }>(this.base, "minijam_getFinalizedContext", []); const result = await this.rpc<{ packageHash: string; submissionHash: string }>(this.workBase, "minijam_submitWorkV1", { context, serviceId: Number(serviceId), serviceCodeHash: service.codeHash, payloadBase64: bytesToBase64(requestBytes), extrinsicsBase64: extrinsics.map(bytesToBase64) }); return { operationId: result.packageHash, kind: "work", status: "submitted", result: {} }; }
