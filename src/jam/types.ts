...
export interface AccountInfo { address: string; name?: string; source?: string; }
...
export interface AccountAdapter { current(): Promise<AccountInfo | null>; connect(): Promise<AccountInfo>; disconnect(): Promise<void>; }
...
export interface DeployInput { blobBase64: string; codeHash: string; minItemGas: number; minMemoGas: number; }
export interface DeployOutput { serviceId: string; operationId?: string; }
export interface InteractInput { serviceId: string; payload: string; }
...
export interface JamSiteManifest { version: 1; generatedAt?: number; index: "/index.html"; files: Record<string, { mime: string; size: number; contentHash: string; chunks: number }>; }
