...
static void dispatch(const uint8_t *payload, size_t payload_size) {
  char op[32], session[160], key[KEY_MAX], map[64], difficulty[64]; if (number_value(payload, payload_size, "version", 0) != 1 || !string_value(payload, payload_size, "op", op, sizeof(op)) || !string_value(payload, payload_size, "sessionId", session, sizeof(session))) return;
  if (strcmp(op, "create_session") == 0) { uint32_t existing_tick = 0, existing_health = 0, existing_ammo = 0, existing_kills = 0, existing_score = 0; int existing_completed = 0; if (load_state(session, &existing_tick, &existing_health, &existing_ammo, &existing_kills, &existing_score, &existing_completed)) return; uint32_t tick = 0, health = 100, ammo = 50, kills = 0, score = 0; int completed = 0; save_state(session, tick, health, ammo, kills, score, completed); if (string_value(payload, payload_size, "map", map, sizeof(map)) && string_value(payload, payload_size, "difficulty", difficulty, sizeof(difficulty))) { char meta[512], runtime[96]; if (!string_value(payload, payload_size, "runtimeVersion", runtime, sizeof(runtime))) runtime[0] = 0; size_t offset = 0; offset = append_text(meta, offset, sizeof(meta), "{\"map\":\""); offset = append_text(meta, offset, sizeof(meta), map); offset = append_text(meta, offset, sizeof(meta), "\",\"difficulty\":\""); offset = append_text(meta, offset, sizeof(meta), difficulty); offset = append_text(meta, offset, sizeof(meta), "\",\"rulesetVersion\":"); offset = append_u32(meta, offset, sizeof(meta), number_value(payload, payload_size, "rulesetVersion", 1)); offset = append_text(meta, offset, sizeof(meta), ",\"runtimeVersion\":\""); offset = append_text(meta, offset, sizeof(meta), runtime); offset = append_text(meta, offset, sizeof(meta), "\"}"); key_for(key, session, ":meta"); store(key, meta); } return; }
